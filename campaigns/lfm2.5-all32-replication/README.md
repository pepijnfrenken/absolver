...
date: 2026-09-02
status: "NEGATIVE (confound isolated) — huihui's exact weight geometry (ALL 32 out-projections at rel_l2 ~0.022-0.030) was replicated faithfully with OUR pipeline, and it does NOT reproduce the published default-greedy compliance with ANY of our three direction sources. The mission's coverage hypothesis is confirmed as NECESSARY (6-attention-only edits never moved chat refusal; the all-32 edits move the refusal surface) but NOT SUFFICIENT: the direction VECTORS are the remaining confound — huihui's own (unpublished) activation-diff vectors from their prompt set, whose effect we partially approached with raw-flavored activation-diff but not enough to pass gates."
...
recommended_next:
  - recover_huihui_direction_from_weights: "The definitive confound test: d is recoverable from the published edit itself. Per out-projection tensor, ΔW = W_huihui − W_base = −α·d·dᵀW is rank-1 → the direction huihui actually used is the top left-singular vector of ΔW (up to scale). Apply THAT vector with our exact all-32 geometry at our rel_l2 window and collect. Removes ALL direction uncertainty — the single highest-information experiment left; keeps everything else (gates, prompts, decoding) from our pipeline."
